...
| A new file dropped straight into `data/` | `data/` is not ignored as a folder, only by pattern (`*.db`, `*.log`, `journal/`, `school/`...). A JSON backup of Ian's class-note text written to `data/` showed as untracked, one `git add -A` from the private repo and one export from the public mirror, where the leak check only knows specific strings and note prose trips none of them | Write private data into a directory the `.gitignore` already names (`data/school/` for class notes, also claimed by `backup.sh`), and confirm with `git check-ignore -v` before trusting it |
...
| A LIKE search that does not escape `%` and `_` | Typing a single `%` returned every row, which reads as a broken search rather than as SQL | Escape the wildcards and pass `ESCAPE`, so a typed wildcard searches for itself. (School notebook search has since left `LIKE` entirely, to fold accents in Python; the lesson stands for any `LIKE` that remains.) |

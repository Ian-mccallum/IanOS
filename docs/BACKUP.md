...
| `launchctl list \| grep ianos.backup` shows **126**, log says `Operation not permitted` | macOS TCC. The repo sits under `~/Desktop`, which is a protected folder, and a LaunchAgent whose program is `/bin/bash` has no access there. The plist runs `.venv/bin/python -c "os.execv('/bin/bash', ...)"` instead: python already has the grant (it is why every other ianOS agent works) and TCC responsibility survives the exec. If it regresses, `make schedule-backup` reinstalls the working plist. |
| No memo, no snapshot, and the log only has `Operation not permitted` | The failure happened **before** `backup.sh` started, so its ERR trap never ran and it could not write the failure memo. The one signal that still fires is Command's agent pulse, which rings when the last success is over 48h old. Trust the ring; `make backup-status` confirms. |
